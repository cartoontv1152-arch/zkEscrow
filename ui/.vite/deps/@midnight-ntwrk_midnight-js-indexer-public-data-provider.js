import "./chunk-OBO3OFS7.js";
import {
  require_browser_ponyfill
} from "./chunk-DHFL4S5D.js";
import {
  FailEntirely,
  FailFallible,
  ...
  SegmentSuccess,
  SucceedEntirely
} from "./chunk-XMK2THOU.js";
import "./chunk-QZY3V4PQ.js";
import "./chunk-WFFFTLIH.js";
import {
  ContractState
} from "./chunk-BDDSC5LD.js";
import {
  assertIsContractAddress,
  warnIfInsecureRemoteUrl
} from "./chunk-HHC3CU4K.js";
import {
  require_buffer
...
var GraphQLDirective = class {
  ...
  constructor(config2) {
    ...
    this.deprecationReason = config2.deprecationReason;
    this.extensions = toObjMap(config2.extensions);
    this.astNode = config2.astNode;
    this.extensionASTNodes = (_config$extensionASTN = config2.extensionASTNodes) !== null && _config$extensionASTN !== void 0 ? _config$extensionASTN : [];
  ...
  */
  watchFragment(options) {
    ...
    if (!Array.isArray(from3)) {
      ...
      return from3 === null ? observable3 : mapObservableFragmentMemoized(observable3, Symbol.for("apollo.transform.individualResult"), (result) => ({
        ...result,
        data: result.data ?? {}
      }));
    }
...
function selectionSetMatchesResult(selectionSet, result, variables) {
  if (isNonNullObject(result)) {
    return isArray2(result) ? result.every((item) => selectionSetMatchesResult(selectionSet, item, variables)) : selectionSet.selections.every((field) => {
...
function extractKeyPath(object, path, extract) {
  ...
  return normalize(path.reduce(function reducer(obj, key) {
    return isArray2(obj) ? obj.map((child) => reducer(child, key)) : obj && extract(obj, key);
  }, object));
}
    ...
    if (fields) {
      Object.keys(fields).forEach((fieldName) => {
        ...
        if (typeof incoming2 === "function") {
          ...
          let { keyArgs, read, merge: merge3, scalar } = incoming2;
          if (scalar) {
...
var StoreReader = class {
  ...
  constructor(config2) {
    ...
    this.prunePartialBoundaries = wrap2((options) => this.prunePartialBoundariesImpl(options), {
      ...
      makeCacheKey: ({ boundaries, context: context2, selectionSet }) => {
        if (supportsResultCaching(context2.store)) {
          return this.keyMaker.lookup(selectionSet, boundaries, context2.streamInfo, context2.deferInfo);
        }
      }
    });
    this.prunePartialStreamArray = wrap2((options) => {
      const { field, context: context2, path } = options;
      if (isStreamField(field, context2.variables)) {
        context2.streamInfo?.lookupArray(path).state.depend();
      }
      return this.prunePartialStreamArrayImpl(options);
    }, {
      max: cacheSizes["inMemoryCache.prunePartialStreamArray"] || 2e4,
      makeCacheKey: ({ field, context: context2, boundaries }) => {
        if (supportsResultCaching(context2.store)) {
          return this.keyMaker.lookup(field, boundaries, context2.streamInfo, context2.deferInfo);
        }
      }
    });
  }
  diffQueryAgainstStore({ store, query, rootId = "ROOT_QUERY", variables, returnPartialData = true, ...options }) {
    ...
    if (returnIncremental && shouldPrune(execResult, context2)) {
    ...
    workSet.forEach((selection) => {
      ...
      if (isField(selection)) {
        ...
        if (fragment && policies.fragmentMatches(fragment, typename)) {
          const isDeferBoundary = isDeferredFragment(selection, context2.variables);
  ...
  execSubSelectedArrayImpl({ field, array, enclosingRef, context: context2 }) {
    ...
    const partialBoundaries = new PartialBoundaries();
    const isStreamed = isStreamField(field, context2.variables);
  ...
  prunePartialBoundariesImpl({ boundaries, context: context2, data, path, selectionSet }) {
    const { variables, lookupFragment, policies } = context2;
    ...
    workSet.forEach((selection) => {
      ...
      let prune = false;
      if (context2.deferInfo && isDeferredFragment(selection, variables)) {
        ...
        prune = !!context2.deferInfo.peekArray(path.concat(label || []));
      }
  ...
  prunePartialStreamArrayImpl({ field, array, boundaries, context: context2, path }) {
    if (!boundaries)
      return { result: array, dataState: "complete" };
    let changed = false;
    let dataState = "complete";
    let pruned = [];
    const state = context2.streamInfo?.peekArray(path)?.state;
...
function shouldPrune({ dataState }, context2) {
  ...
  if (dataState === "complete" || dataState === "streaming") {
    ...
    (context2.streamInfo || // The network hasn't delivered these @defer boundaries yet, so prune
  ...
  processSelectionSet({
    ...
    this.flattenFields(
      ...
      if (value !== void 0) {
        ...
        } else if (context3.extensions?.[streamInfoSymbol] && Array.isArray(incomingValue) && isStreamField(field, context3.variables)) {
          childTree.info = {
            field,
            typename,
  ...
  applyMerges(mergeTree, existing, incoming, context2, getStorageArgs) {
    if (mergeTree.map.size && !isReference(incoming)) {
      ...
      const getValue = (from3, name) => {
        return isArray2(from3) ? typeof name === "number" ? from3[name] : void 0 : context2.store.getFieldValue(from3, String(name));
      };
      ...
      if (changedFields) {
        incoming = isArray2(i) ? i.slice(0) : { ...i };
...
var QueryManager = class {
  ...
  fetchQueryByPolicy({ query, variables, fetchPolicy, errorPolicy, returnPartialData, context: context2, networkStatus }, { cacheWriteBehavior, onCacheHit, queryInfo, observableQuery, exposeExtensions }) {
    const readCache = () => {
    ...
    const resultsFromCache = (diff, networkStatus2) => {
      ...
      if (
        ...
        return from(this.localState.execute({
          ...
          kind: "N",
          value: toResult(resolved2.data || void 0, diff.complete ? "complete" : resolved2.data ? "partial" : "empty"),
          // Always attach the variables used for this fetch so @export
          // resolution can update ObservableQuery.options.variables and
          // resubscribe the cache watch under the correct variable set.
          resolvedVariables: variables,
...
var ApolloClient = class {
  ...
  */
  ...
  watchFragment(options) {
    ...
    if (__DEV__) {
      return mapObservableFragmentMemoized(observable2, Symbol.for("apollo.transform.dev.mask"), (result) => ({
        ...result,
...
function buildDelayFunction(delayOptions) {
  const { initial = 300, jitter = true, max: max2 = Infinity } = delayOptions || {};
  const baseDelay = jitter ? initial : initial / 2;
  return function delayFunction(count2) {
    let delay2 = Math.min(max2, baseDelay * 2 ** count2);
    if (jitter) {
      delay2 = Math.random() * delay2;
    }
    return delay2;
  };
}
...
function buildRetryFunction(retryOptions) {
  const { retryIf, max: max2 = 5 } = retryOptions || {};
  return function retryFunction(count2, operation, error) {
    if (count2 >= max2)
      return false;
...
var RetryLink = class extends ApolloLink {
  ...
  constructor(options) {
    super();
    const { attempts, delay: delay2 } = options || {};
    this.delayFor = typeof delay2 === "function" ? delay2 : buildDelayFunction(delay2);
...
function createClient(options) {
  ...
  return {
    ...
    iterate(request) {
      ...
      const iterator2 = (async function* iterator22() {
        for (; ; ) {
          if (!pending.length) {
...
var IndexerDataError = class _IndexerDataError extends IndexerError {
  ...
  constructor(context2) {
    super(_IndexerDataError.formatMessage(context2));
  ...
  static formatMessage(context2) {
    switch (context2.kind) {
      case "unknown-status":
        return `Unexpected transaction status value: ${context2.value}`;
      case "missing-contract-action":
        return `Deploy transaction does not contain a contract action for address ${context2.contractAddress}`;
      case "missing-identifier":
        return `Transaction missing identifier for contract action at address ${context2.contractAddress} (actionIndex=${context2.actionIndex}, identifiers.length=${context2.identifiersLength})`;
    }
  }
};
...
var toByteArray = (s) => import_buffer2.Buffer.from(s, "hex");
...
var indexerPublicDataProviderInternal = (queryURL, subscriptionURL, webSocketImpl = void 0) => {
  ...
  return {
    async queryContractState(address, config2) {
      let offset;
      if (config2) {
        offset = {
          blockOffset: config2.type === "blockHeight" ? { height: config2.blockHeight } : { hash: config2.blockHash }
        };
      } else {
        offset = null;
      }
    ...
    async queryZSwapAndContractState(address, config2) {
      let offset;
      if (config2) {
        offset = {
          blockOffset: config2.type === "blockHeight" ? { height: config2.blockHeight } : { hash: config2.blockHash }
    ...
    async queryUnshieldedBalances(address, config2) {
      let offset;
      if (config2) {
        offset = {
          blockOffset: config2.type === "blockHeight" ? { height: config2.blockHeight } : { hash: config2.blockHash }
    ...
    contractStateObservable(contractAddress, config2 = { type: "latest" }) {
      if (config2.type === "txId") {
        const contractStates = transactionIdToTransaction$(apolloClient)(config2.txId).pipe(filter(isRegularTransaction), concatMap(transactionToContractState$(config2.txId)));
        return config2.inclusive ?? true ? contractStates : contractStates.pipe(skip(1));
      }
      if (config2.type === "latest") {
      ...
      const offset = config2.type === "blockHash" ? { hash: config2.blockHash } : { height: config2.blockHeight };
      const blocks = waitForBlockToAppear(apolloClient)(offset).pipe(concatMap(() => blockOffsetToBlock$(apolloClient)(offset)));
      const maybeShortenedBlocks = config2.type === "blockHeight" || config2.type === "blockHash" ? iif(() => config2.inclusive ?? true, blocks, blocks.pipe(skip(1))) : blocks;
      return maybeShortenedBlocks.pipe(concatMap(blockToContractState$(contractAddress)));
    },
    unshieldedBalancesObservable(contractAddress, config2 = { type: "latest" }) {
      if (config2.type === "txId") {
      ...
      return config2.type === "blockHeight" || config2.type === "blockHash" ? iif(() => config2.inclusive ?? true, balances, balances.pipe(skip(1))) : balances;
    }
  };
};
var indexerPublicDataProvider = (queryURL, subscriptionURL, webSocketImpl = void 0) => {
  ...
  return {
    contractStateObservable(contractAddress, config2) {
      assertIsContractAddress(contractAddress);
      return publicDataProvider.contractStateObservable(contractAddress, config2);
    },
    queryContractState(contractAddress, config2) {
      assertIsContractAddress(contractAddress);
      return publicDataProvider.queryContractState(contractAddress, config2);
    },
    queryDeployContractState(contractAddress) {
      assertIsContractAddress(contractAddress);
      return publicDataProvider.queryDeployContractState(contractAddress);
    },
    queryZSwapAndContractState(contractAddress, config2) {
      assertIsContractAddress(contractAddress);
      return publicDataProvider.queryZSwapAndContractState(contractAddress, config2);
    },
    queryUnshieldedBalances(contractAddress, config2) {
      assertIsContractAddress(contractAddress);
      return publicDataProvider.queryUnshieldedBalances(contractAddress, config2);
    },
    ...
    unshieldedBalancesObservable(contractAddress, config2) {
      assertIsContractAddress(contractAddress);
      return publicDataProvider.unshieldedBalancesObservable(contractAddress, config2);
    }
  };
};
export {
